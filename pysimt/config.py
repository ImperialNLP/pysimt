...
TRAIN_DEFAULTS = {
    ...
    'tf_model_dim': None,        # The transformer model dimension, needed for the noam scheduler.
    'lr_warmup_steps': 0,        # The scheduler warmup steps
    ...
    'adam_betas': (0.9, 0.999),  # The betas parameter for Adam.
...
class Options:
        ...
        if override_list is not None:
            ...
            for section, ov_dict in overrides.items():
                for key, value in ov_dict.items():
                    if '.' in key:
                        key_outer, key_inner = key.split('.')
                        obj.__dict__[section][key_outer][key_inner] = value
                    else:
    ...
    def __init__(self, filename, overrides=None):
        self._parser = ConfigParser(interpolation=ExtendedInterpolation(), allow_no_value=True)
